...
export default function Home() {
  return <>
    ...
    <main>
      ...
      <section className="full-band workflow-band"><div className="section-inner workflow-section"><div id="services" className="anchor-target operation-map"><p className="section-label">현장이 움직이는 순서</p><p className="operation-lead">한 객실을 위해 7개의 역할이 맞물려 움직입니다.</p><div className="operation-control"><strong>데스크</strong><span>일정 확인 · 업무 배정 · 현장 이슈 관리</span></div><div className="operation-stages">{operationStages.map(([icon, title, team]) => <div className="operation-stage" key={title}><Icon name={icon}/><strong className="operation-stage-title">{title}</strong><span className="operation-stage-team">{team}</span></div>)}</div><p className="operation-crosscheck">배급 → 클리닝 → 검수까지 <strong>크로스체크</strong>합니다.</p><div className="operation-support"><div className="operation-support-teams"><span>세탁팀</span><span aria-hidden="true">·</span><span>배달팀</span><span aria-hidden="true">·</span><span>수거팀</span></div><p>침구와 비품의 세탁·이동·회수를 이어갑니다.</p></div></div><div id="operations" className="anchor-target operation-strip" aria-label="운영 근거">{operationFacts.map(([value, detail]) => <div key={value}><strong>{value}</strong><span>{detail}</span></div>)}</div><section id="laundry" className="anchor-target laundry-editorial"><h2 className="sr-only">침구 운영</h2><div className="laundry-main"><span className="laundry-label">침구 운영</span><div className="laundry-scope"><p><span>수거</span><span className="scope-tail"><span className="scope-separator" aria-hidden="true">·</span><strong>세탁실·세탁공장</strong></span></p><p><span>분류·보관</span><span className="scope-tail"><span className="scope-separator" aria-hidden="true">·</span><strong>객실별 재배포</strong></span></p></div></div><aside className="special-care"><span className="special-care-label">별도 공정</span><Icon name="laundry"/><h3>특수오염 처리</h3><p>일반 세탁이 어려운 오염은<br />별도 공정으로 확인합니다.</p></aside></section></div></section>
