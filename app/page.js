...
export default function Home() {
  return <>
    ...
    <main>
      ...
      <section id="showcase" className="section"><p className="eyebrow">System</p><h2>현장에서 확인한 내용이 시스템 기록으로 남습니다.</h2><p>일정, 완료 사진, 체크리스트, 시설 이슈와 비용 내역을 객실별로 남겨 호스트가 필요한 순간에 다시 볼 수 있게 합니다.</p><div className="operation-strip"><div><h3>여러 담당자가 각 단계를 나눠 확인합니다.</h3><p>데스크에서 접수한 일정은 침구 배송과 배급, 객실 클리닝, 검수, 수거와 세탁으로 이어집니다. 각 단계의 담당자가 앞뒤 작업을 다시 확인해 누락이나 현장 문제를 빠르게 공유합니다.</p></div><div className="team-flow">{operationTeams.map((team, index) => <span key={team}>{team}{index < operationTeams.length - 1 ? <b>→</b> : null}</span>)}</div><div className="operation-notes"><strong>약 60명 규모의 클리너 풀 운영</strong><p>객실 수와 지역, 일정과 작업 난이도를 확인해 현장에 맞는 인력을 배정합니다.</p><strong>배급 → 클리닝 → 검수 단계별 크로스체크</strong><p>침구와 비품이 제대로 전달됐는지, 청소가 기준에 맞게 끝났는지, 추가로 공유할 오염이나 시설 문제가 있는지를 단계별로 다시 봅니다.</p></div></div><div className="showcase-grid">{showcases.map(([key,title,body]) => <article className="showcase" key={key} data-image-key={key}><ShowcaseImage type={key} title={title}/><div><h3>{title}</h3><p>{body}</p></div></article>)}</div></section>
