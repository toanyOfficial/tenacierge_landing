...
export default function Home() {
  return <>
    ...
    <main>
      ...
      <section className="full-band workflow-band"><div className="section-inner workflow-section"><div id="services" className="anchor-target"><p className="section-label">현장이 움직이는 순서</p><h2>다음 체크인까지,<br />이 순서로 움직입니다.</h2><ol className="field-flow">{fieldSteps.map(([icon, title, detail], index) => <li key={title}><span className="flow-number">0{index + 1}</span><Icon name={icon}/><strong>{title}</strong><small>{detail}</small></li>)}</ol></div><div id="operations" className="anchor-target operation-strip" aria-label="운영 근거">{operationFacts.map(([value, detail]) => <div key={value}><strong>{value}</strong><span>{detail}</span></div>)}</div><div id="laundry" className="anchor-target laundry-strip"><header className="laundry-heading"><p className="section-label">침구 운영</p><h3>자체 세탁시설에서 세탁하고,<br />객실 일정에 맞춰 다시 배포합니다.</h3></header><div className="laundry-process-grid"><div className="standard-laundry"><p className="laundry-process-label">자체 세탁시설</p><ol className="laundry-flow">{laundrySteps.map(([icon, title], index) => <li key={title}><span className="laundry-number">0{index + 1}</span><Icon name={icon}/><strong>{title}</strong></li>)}</ol></div><aside className="stain-process"><p className="laundry-process-label">별도 공정</p><h4>특수오염 처리</h4><p>일반 세탁이 어려운 오염은<br />별도 공정으로 확인합니다.</p></aside></div></div></div></section>
